...
export function refreshPersonaCache() { _personaCache = null; }
const OPENROUTER_URL = 'https://openrouter.ai/api/v1/chat/completions';
// ─── Core fetch (single attempt) ─────────────────────────────────────────────
async function fetchLLM(messages, tools, model, stream) {
...
export async function callLLM(messages, tools = [], modelOverride, stream = false) {
...
export async function quickLLM(prompt, modelOverride) {
    const res = await callLLM([{ role: 'user', content: prompt }], [], modelOverride, false);
    return res.content;
}
