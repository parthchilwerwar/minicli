...
export function loadOpportunities() {
    ...
    if (!fs.existsSync(OPP_FILE))
        return [];
    try {
        return z.array(OpportunitySchema).parse(JSON.parse(fs.readFileSync(OPP_FILE, 'utf-8')));
    }
    catch {
        return [];
    }
}
export function saveOpportunities(o) { ensure(); fs.writeFileSync(OPP_FILE, JSON.stringify(o, null, 2)); }
export function loadContentLog() {
    ensure();
    if (!fs.existsSync(CONTENT_FILE))
        return [];
    try {
        return z.array(ContentLogSchema).parse(JSON.parse(fs.readFileSync(CONTENT_FILE, 'utf-8')));
    }
    catch {
        return [];
    }
}
export function saveContentLog(c) { ensure(); fs.writeFileSync(CONTENT_FILE, JSON.stringify(c, null, 2)); }
// ─── Agent ────────────────────────────────────────────────────────────────────
export class OpportunityContentAgent {
    ...
    async stop() { this.digestJob?.stop(); this.contentJob?.stop(); }
    ...
    async weeklyDigest() {
        try {
            ...
            console.error('[opportunity] digest error:', err instanceof Error ? err.message : err);
        }
    }
...
//# sourceMappingURL=opportunity-content.js.map