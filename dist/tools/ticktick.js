...
function loadToken() {
    ...
    if (!token)
        throw new Error('TICKTICK_ACCESS_TOKEN not set in .env');
    return token;
}
async function ttFetch(path, method = 'GET', body) {
...
async function fetchAllTasks() {
    const projects = (await ttFetch('/project'));
    const allTasks = [];
    for (const proj of projects) {
        try {
            const data = (await ttFetch(`/project/${proj.id}/data`));
            if (data.tasks) {
...
// ─── Tools ───────────────────────────────────────────────────────────────────
export const ticktickGetTodayTool = {
...
export const ticktickGetAllTool = {
    ...
    async execute(args) {
        ...
        return active.map((t) => `• [P${t.priority}] ${t.title}${t.dueDate ? ` (due ${t.dueDate.slice(0, 10)})` : ''}`).join('\n');
    },
};
export const ticktickCreateTaskTool = {
    ...
    async execute(args) {
        ...
        const task = (await ttFetch('/task', 'POST', body));
        return `Task created: "${task.title}" (id: ${task.id})`;
    },
};
export const ticktickCompleteTaskTool = {
...
export const ticktickGetProjectsTool = {
...
//# sourceMappingURL=ticktick.js.map