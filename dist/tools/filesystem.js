...
export const fsReadFileTool = {
    ...
    async execute(args) {
        ...
        if (isBinary(safe))
            return `ERROR: Cannot read binary file: ${basename(safe)}`;
...
export const fsFindTool = {
    ...
    async execute(args) {
        ...
        if (!searchIn || searchIn === 'all')
            dirs.push(...getAllowedRoots());
        else if (searchIn === 'vault') {
            const v = getVaultPath();
            if (v)
                dirs.push(resolve(v));
        }
        else if (searchIn === 'desktop') {
            const d = getDesktopPath();
            if (d)
                dirs.push(resolve(d));
        }
        else if (searchIn === 'downloads') {
            const d = getDownloadsPath();
            if (d)
                dirs.push(resolve(d));
        }
        if (!dirs.length)
            return 'No search directories configured.';
        // Search all root dirs in parallel for speed
        const perRoot = await Promise.all(dirs.filter((d) => existsSync(d)).map((d) => findFiles(d, query)));
...
export const fsGetStructureTool = {
...
export const vaultListTool = {
    ...
    async execute() {
        ...
        if (!vault)
            return 'ERROR: VAULT_PATH not configured in .env';
...
export const vaultReadTool = {
    ...
    async execute(args) {
        ...
        if (!match)
            return `Note "${filename}" not found in vault.`;
