import { z } from 'zod';
import { loadNotes, saveNote, deleteNote, searchNotes, filterByTag, } from '../memory.js';
...
export const listNotesTool = {
    ...
    async execute(args) {
        ...
        try {
            ...
            return notes
                .map((n) => `[${n.id}] ${n.text.slice(0, 60)} | ${n.tags.map((t) => `#${t}`).join(' ')} | ${new Date(n.ts).toLocaleDateString()}`)
                .join('\n');
        }
        catch (err) {
            return `ERROR: ${err instanceof Error ? err.message : String(err)}`;
