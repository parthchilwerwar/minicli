...
export const gitLogTool = {
    ...
    async execute(args) {
        ...
        try {
            ...
            const lines = log.all.map((c, i) => `${i + 1}. ${c.hash.slice(0, 7)} | ${c.message.slice(0, 60)} | ${c.author_name} | ${c.date}`);
            return lines.join('\n') || 'No commits found.';
        }
        catch (err) {
            return `ERROR: ${err instanceof Error ? err.message : String(err)}`;
...
export async function getRawGitLog(n) {
