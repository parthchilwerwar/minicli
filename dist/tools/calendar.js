...
function loadToken() {
    if (!existsSync(TOKEN_PATH))
        throw new Error('Google Calendar not authenticated. Run: mini calendar auth');
...
// ─── OAuth2 Flow ─────────────────────────────────────────────────────────────
export async function calendarAuth() {
...
// ─── Tools ───────────────────────────────────────────────────────────────────
export const calendarGetTodayTool = {
    ...
    async execute() {
        ...
        const data = (await calFetch(`/calendars/primary/events${q}`));
...
export const calendarGetWeekTool = {
...
export const calendarFreeSlotsTool = {
    ...
    async execute(args) {
        ...
        if (!slots.length)
            return `No free slots of ${durationMinutes}+ minutes found today.`;
