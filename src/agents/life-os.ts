...
import { sendToTelegram, broadcast } from './base.js';
...
export function loadTasks(): Task[] {
  ...
  try { return z.array(TaskSchema).parse(JSON.parse(fs.readFileSync(TASKS_FILE, 'utf-8'))); }
  catch { return []; }
}
export function saveTasks(t: Task[]): void { ensure(); fs.writeFileSync(TASKS_FILE, JSON.stringify(t, null, 2)); }

export function loadHabits(): Habit[] {
  ensure();
  if (!fs.existsSync(HABITS_FILE)) return [];
  try { return z.array(HabitSchema).parse(JSON.parse(fs.readFileSync(HABITS_FILE, 'utf-8'))); }
  catch { return []; }
}
export function saveHabits(h: Habit[]): void { ensure(); fs.writeFileSync(HABITS_FILE, JSON.stringify(h, null, 2)); }

export function loadReminders(): Reminder[] {
  ensure();
  if (!fs.existsSync(REMINDERS_FILE)) return [];
  try { return z.array(ReminderSchema).parse(JSON.parse(fs.readFileSync(REMINDERS_FILE, 'utf-8'))); }
  catch { return []; }
}
export function saveReminders(r: Reminder[]): void { ensure(); fs.writeFileSync(REMINDERS_FILE, JSON.stringify(r, null, 2)); }
...
export class LifeOSAgent implements Agent {
  name = 'Life OS';
  ...
  private habitJob: cron.ScheduledTask | null = null;
  ...
  async stop(): Promise<void> {
    this.morningJob?.stop(); this.nightJob?.stop(); this.habitJob?.stop();
    if (this.reminderInterval) clearInterval(this.reminderInterval);
  }

  // ── Behavior B: Capture intercept ──────────────────────────────────────────

  async intercept(message: string): Promise<{ handled: boolean; response?: string }> {
    try {
      const now = new Date();
      ...
      if (c.type === 'query') return { handled: false };
  ...
  private async morningBrief(): Promise<void> {
    try {
    ...
    } catch (err) { console.error('[life-os] morning brief error:', err instanceof Error ? err.message : err); }
  }

  // ── Behavior C: Habit nudge ────────────────────────────────────────────────

  private async habitNudge(): Promise<void> {
    try {
      const habits = loadHabits();
      const now = Date.now();
      let changed = false;
      ...
      for (const h of habits) {
        ...
        if (h.nudgeSentAt) {
          ...
          if ((now - lastNudge) < 24 * 60 * 60 * 1000) continue;
        }
  ...
  private async nightRecap(): Promise<void> {
    try {
      const name = process.env['LIFE_OS_NAME'] ?? 'Parth';
    ...
    } catch (err) { console.error('[life-os] night recap error:', err instanceof Error ? err.message : err); }
  }
