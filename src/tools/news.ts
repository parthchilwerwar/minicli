...
export const newsFetchRssTool: ToolDefinition = {
  name: 'news_fetch_rss',
...
export const newsSummariseTodayTool: ToolDefinition = {
  ...
  async execute(args) {
    ...
    if (!allItems.length) return 'Could not fetch any news feeds.';
...
export const newsSearchHeadlinesTool: ToolDefinition = {
  ...
  async execute(args) {
    ...
    if (!matches.length) return `No headlines matching "${query}".`;
