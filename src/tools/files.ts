...
export const readFileTool: ToolDefinition = {
  ...
  async execute(args) {
    ...
    try {
      ...
      if (isBinary(safe)) return `ERROR: Cannot read binary file: ${path}`;
...
export const writeFileTool: ToolDefinition = {
  name: 'write_file',
  description:
    'Write content to a file inside an allowed root (vault/notes/desktop/downloads). Creates parent directories. Refuses to overwrite binary files.',
  parameters: WriteFileParams,
  async execute(args) {
    const { path, content } = WriteFileParams.parse(args);
    try {
      const safe = assertAllowed(path);
      if (isBinary(safe)) return `ERROR: Refusing to write binary file extension: ${path}`;
