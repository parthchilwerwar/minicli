...
import { success, error, info, warn } from './ui.js';
...
export async function doDashboard(): Promise<void> {
  ...
  try {
    const location = typeof ctx.preferences === 'object' && ctx.preferences !== null
      ? (ctx.preferences as Record<string, string>)['location'] ?? ''
      : '';
...
export async function doDashboardText(): Promise<string> {
  const ctx  = loadContext();
  const name = ctx.name ?? '';
  const now  = new Date();
  const lines: string[] = [];
  ...
  lines.push('📋 ROUTINES');
  const routines = loadRoutines();
  if (!routines.length) lines.push('  No routines configured.');
  else routines.forEach((r) => lines.push(`  · ${r.desc} (${r.cronExpr ?? 'manual'})`));
  lines.push('');

  lines.push('📝 RECENT NOTES');
  const notes = loadNotes().slice(-3).reverse();
  if (!notes.length) lines.push('  No notes yet.');
  else notes.forEach((n) => lines.push(`  · ${n.text.slice(0, 72)}`));
  lines.push('');
  ...
  try {
    ...
    const tasks = await quickLLM(
      `Based on this context:\n${ctxSummary || 'No context set.'}\n\nSuggest exactly 3 short actionable tasks for today. Output ONLY a numbered list, nothing else.`
    );
    tasks.split('\n').filter((l) => /^\d+\./.test(l.trim())).slice(0, 3).forEach((t) =>
...
export async function contextSet(): Promise<void> {
  const existing = loadContext();

  const answers = await inquirer.prompt<{
    name: string; bio: string; goals: string; habits: string; location: string;
  }>([
    ...
    { type: 'input', name: 'location', message: 'Location (for weather):',   default: (typeof existing.preferences === 'object' && existing.preferences !== null ? (existing.preferences as Record<string,string>)['location'] : '') ?? '' },
  ]);
...
export function contextShow(): void {
  const ctx = loadContext();
  ...
  if (ctx.name)   console.log('  ' + DIM('Name: ')    + WHITE(ctx.name));
  if (ctx.bio)    console.log('  ' + DIM('Bio: ')     + WHITE(ctx.bio));
  if (ctx.goals)  console.log('  ' + DIM('Goals: ')   + WHITE(Array.isArray(ctx.goals)  ? ctx.goals.join(', ')  : ctx.goals));
  if (ctx.habits) console.log('  ' + DIM('Habits: ')  + WHITE(Array.isArray(ctx.habits) ? ctx.habits.join(', ') : ctx.habits));
  ...
  if (ctx.updatedAt) console.log('  ' + DIM('Updated: ') + DIM(new Date(ctx.updatedAt).toLocaleString()));
  console.log('');
}

export function contextClear(): void {
  clearContext();
  success('Context cleared.');
}

export function contextExport(): void {
  const ctx = loadContext();
  console.log(JSON.stringify(ctx, null, 2));
}

export function contextLoad(file: string): void {
  try {
...
export async function contextAuto(): Promise<void> {
  ...
  try {
    ...
    const res   = await quickLLM(
      'Analyze the user profile implied by this CLI session. Output ONLY valid JSON with keys: name, bio, goals (array), habits (array), preferences (object). No markdown.'
    );
