...
function jsonReply(res: http.ServerResponse, status: number, data: unknown): void {
  res.writeHead(status, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify(data));
}

function errReply(res: http.ServerResponse, code: number, message: string): void {
...
async function handleRoute(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  const url = req.url ?? '/';
  ...
  if (method === 'GET' && url === '/vault/list') {
    ...
    if (!tool) { errReply(res, 500, 'vault_list tool not found'); return; }
  ...
  if (method === 'POST' && url === '/vault/read') {
    ...
    if (!tool) { errReply(res, 500, 'vault_read tool not found'); return; }
  ...
  if (method === 'GET' && url?.startsWith('/fs/list')) {
    ...
    if (!path) { errReply(res, 400, 'Missing path parameter'); return; }
    const tool = ALL_TOOLS.find((t) => t.name === 'fs_list_dir');
    if (!tool) { errReply(res, 500, 'fs_list_dir tool not found'); return; }
  ...
  if (method === 'POST' && url === '/fs/read') {
    ...
    if (!tool) { errReply(res, 500, 'fs_read_file tool not found'); return; }
  ...
  if (method === 'POST' && url === '/fs/find') {
    ...
    if (!tool) { errReply(res, 500, 'fs_find tool not found'); return; }
...
export function startBridgeServer(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    server.on('error', (err: NodeJS.ErrnoException) => {
      if (err.code === 'EADDRINUSE') reject(new Error(`Bridge port ${port} already in use`));
      else reject(err);
    });
