...
export async function runAgent(
  ...
  while (steps < MAX_STEPS) {
    if (steps > 0 && stream) process.stdout.write('  '); // Indent continued responses after a spinner
    ...
    for (const toolCall of response.tool_calls) {
      ...
      try   { args = JSON.parse(toolCall.function.arguments) as Record<string, unknown>; }
      catch { args = {}; }
...
export async function runSupervisedPlan(task: string, tools: ToolDefinition[]): Promise<void> {
